...
export const CurtainMenuPage = ({ open, setOpen }) => {
    ...
    return (
        <Menu open={open}>
            <AiOutlineClose onClick={() => setOpen((prevState) => !prevState)} className='absolute top-4 right-4 font-bold text-teal-500 cursor-pointer' size={'2.5rem'} />
            <MenuContainer>
...
const style = {
    ...
    item: `relative font-medium text-2xl  before:absolute before:-bottom-1 before:h-0.5 before:w-full before:scale-x-0 before:bg-teal-500 before:transition hover:before:scale-x-100`,
    menu: {
        open: `h-full w-full `,
        close: `w-0 h-full`,
